...
## Структура файлов:
- файлы с кодом: <a href="https://github.com/NikaFortem/project_crypto__fen_2023/blob/main/parcing_binance.ipynb" target="_blank">parcing_binance.ipynb</a> / <a href="https://github.com/NikaFortem/project_crypto__fen_2023/blob/main/data_analysis.ipynb" target="_blank">data_analysis.ipynb</a> 
...
В рамках нашего проекта мы проанализировали котировки топ-68 криптовалют за период с 01.01.2020 по 10.05.2023, полученные при помощи  <a href="https://github.com/NikaFortem/project_crypto__fen_2023/blob/main/parcing_binance.ipynb" target="_blank">парсинга</a> с криптовалютной
...
Код для парсинга можно найти <a href="https://github.com/NikaFortem/project_crypto__fen_2023/blob/main/parcing_binance.ipynb" target="_blank">тут</a>.

Мы подключились к криптовалютной бирже Binance через API-ключи. С их помощью получили все торгуемые криптовалютные пары. Чтобы проанализировать больше криптовалют, мы рассматривали криптовалютные пары типа "криптовалюта/USDT", поскольку не все криптовалюты торгуются в фиатных валютах. Далее собрали котировки для получившихся криптовалютных пар с 01.01.2020 по 10.05.2023. Для более корректного анализа мы взяли топ-68 криптовалют, так как другие криптовалюты более новые, они существуют менее 2 лет, и данных по их котировкам недостаточно для исследования. Получившиеся данные мы представили в <a href="https://github.com/NikaFortem/project_crypto__fen_2023/blob/main/df_price.csv" target="_blank">таблице</a>, где по столбцам представлены валютные пары типа "криптовалюта/USDT", а по строкам - их ежедневные цены начиная с 01.01.2020 и до 10.05.2023.
...
- $σ_p$ - стандартное отклонение доходности анализируемого актива
   ...
   (https://data.nasdaq.com/data/USTREASURY/YIELD-treasury-yield-curve-rates). Эти данные представлены в таблице <a href="https://github.com/NikaFortem/project_crypto__fen_2023/blob/main/USTREASURY-YIELD.csv" target="_blank">USTREASURY-YIELD.csv</a> .
